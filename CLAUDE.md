...
| `clickup_work/picker.py` | `TicketPickerApp` — Textual TUI replacing the fzf/numbered picker | Filterable, grouped by folder, color-coded priority. Default surface; fzf falls back when `--no-tui` is set. `a` opens `actions_screen.TicketActionsScreen` when an `ActionsContext` is supplied (the cli does, the fzf/numbered fallbacks don't). Accepts a `theme=` kwarg; when `"ben10"` it mounts an `OmnitrixBanner` at the top and applies the Omnitrix-green palette via `themes.apply_theme`. |
| `clickup_work/themes.py` | `BEN10_THEME` (Textual `Theme`), `OmnitrixBanner` (Static widget — scrolling-binary marquee with "BEN 10 · OMNITRIX" hourglass title), `apply_theme(app, name)` helper | Centralised so picker/plan/post-flow/workload all stay free of palette details. Add new themes by appending to `_THEMES` and the `--theme` choice list updates automatically. Theme name flows in via `--theme NAME` flag → `$CLICKUP_WORK_THEME` env var → `config.theme` → kwargs into each App's `__init__`. Unknown names silently fall back to default. `apply_theme` always registers every known theme so they appear in Textual's command palette (`Ctrl+P → Change theme`); a `watch(app, "theme", _persist_theme_change, init=False)` watcher saves any palette pick back to `config.toml`. Built-in `textual-*` themes are mapped to "default" (cleared) on persist so picking textual-dark removes the saved line. |
| `clickup_work/actions_screen.py` | `TicketActionsScreen` — per-ticket action menu: status (`s`), priority (`p`), due date (`d`), start date (`b`), estimate (`e`), track time (`t`), rename (`r`), description (`D` — opens `$EDITOR` via `app.suspend()`), tags (`T`), assignees (`A`), subtasks (`S`), comments (`c`), time-entry history (`H`), open (`o`), send-to-Claude (`g`). Sub-modals: `RenamePrompt`, `DatePrompt`, `PriorityPrompt`, `TagPrompt`, `AssigneesScreen`, `SubtasksScreen` + `SubtaskNamePrompt`, `TimeEntriesScreen` + `EditTimeEntryPrompt` + `ConfirmPrompt`, `CommentsScreen` + `CommentComposeScreen`. `ActionsContext` dataclass holds the client + ids. | Reached by pressing `a` in the picker. Reuses `StatusPrompt` + `EstimatePrompt` from `tui.py` and `MemberPrompt` from `post_flow.py`. Returns the sentinel `SEND_TO_CLAUDE` so the picker exits with that ticket. Date inputs are parsed by `_parse_date_input` (`2026-05-10`, `+3d`, `today`, `tomorrow`, or `clear`/empty). Note: don't pass `value=` to a `Select` whose initial value matches an option — Textual fires `Select.Changed` on mount and the modal auto-dismisses; use `prompt="..."` instead. (Existing `StatusPrompt` in `tui.py` still does this — it works in production but trips `app.run_test` pilots.) |
